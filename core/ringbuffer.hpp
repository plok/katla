...
namespace katla
{
	...
	template<typename T, size_t buffer_size = 16, bool fake_tso = false, size_t cacheline_size = 0, typename index_t = size_t>
		...
		public:
			 ...
			 * \return True if data was inserted
             *
             * SES: TODO ok this might be a very bad idea, lets find out :)  
			 */
			bool insertByCallback(std::function<void(T& data)> callback)
			{
				index_t tmp_head = head.load(std::memory_order_relaxed);
				index_t tmp_tail = tail.load(index_acquire_barrier);

				// katla::printInfo("insert head {}: {}, tail: {}", reinterpret_cast<size_t>(&head), tmp_head, tmp_tail);

				if((tmp_head - tmp_tail) == buffer_size) {
					// katla::printInfo("insert: buffer full");
					return false;
                }

                callback(data_buff[tmp_head++ & buffer_mask]);
			...
			bool insertFromCallbackWhenAvailable(T (*get_data_callback)())
			{
				...
				// katla::printInfo("insert: head: {}, tail: {}", tmp_head, tmp_tail);
			 ...
			 */
			std::optional<T*> peek() {
				// katla::printInfo("peaek: lock");

				index_t tmp_head = head.load(std::memory_order_relaxed);

				// katla::printInfo("peaek {}: head: {}", reinterpret_cast<size_t>(&head), tmp_head);
				index_t tmp_tail = tail.load(index_acquire_barrier);

				// katla::printInfo("peaek {}: head: {}, tail: {}", reinterpret_cast<size_t>(&head), tmp_head, tmp_tail);

				if(tmp_tail == tmp_head) {
					// katla::printInfo("peek empty");
					return {};
                }

				// katla::printInfo("peek");
