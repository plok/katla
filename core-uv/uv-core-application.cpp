...
UvCoreApplication::UvCoreApplication() :
    m_interruptSignalHandler(m_eventLoop),
    m_terminateSignalHandler(m_eventLoop),
    m_hangupSignalHandler(m_eventLoop)
{
}
...
outcome::result<void, Error> UvCoreApplication::init()
{
    ...
    error = m_interruptSignalHandler.start(Signal::Interrupt, [this]() {
        ...
        if (!closeResult) {
            katla::print(stderr, "{}\n    {}-{}", closeResult.error().message(), closeResult.error().description(), closeResult.error().info());
        }
    });
...
outcome::result<void, Error> UvCoreApplication::run()
{
...
outcome::result<void, Error> UvCoreApplication::close()
{
    ...
    if (!result) {
        katla::print(stderr, "Failed closing event-loop: {}\n    {}-{}\n",
...
EventLoop& UvCoreApplication::eventLoop() {
    return m_eventLoop;
}
UvEventLoop& UvCoreApplication::uvEventLoop() {
    return m_eventLoop;
}
